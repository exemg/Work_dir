...
if strcmpi(rawnetbit,'raw')
    dirlist_csv = ls('*Latplot*raw*.csv');  % list of .csv files with the "*yyy" particles in the title
    suffix = strcat('raw_NG',num2str(NG));
elseif strcmpi(rawnetbit,'net')
    dirlist_csv = ls('*Latplot*net_2*.csv');
    suffix = strcat('net_NG',num2str(NG));
else
...
sv_pth_gen = strcat('Analysis_',suffix,'_',dtst,'\');
...
if Ncsv == 1
    pth_loc_id = dirlist_csv(end-15:end-4);
...
for ixT = 1:Ncsv
    ...
    for ix = 1:N
        ...
        D = 1.5; % this is the initial value for the width of the peaks
...
%                 fo.StartPoint = [Bkg_0 Bkg_1 P(1) WS(1) LOCS(1) P(2) WS(2) LOCS(2) P(3) WS(3) LOCS(3)];
                fo.StartPoint = [Bkg_0 Bkg_1 P(1) D LOCS(1) P(2) D LOCS(2) P(3) D LOCS(3)];
            elseif NG == 2
                [LOCS,isort2] = sort(LOCS,'ascend'); % let's sort them in depth order
                PKS = PK(isort2);
                WS = W(isort2);
...
%         idx_sel = and(idx_sel,pkswdthserr<errfact*pkswdths);   % We ignore values with too big an uncertainty
        ...
        idx_sel = and(idx_sel,mean(pkswdths)-pkswdths < selstdfactor*std(pkswdths));
    else
        idx_sel = (fitR2>0.75);  % we select all the beads whose fit has R^2 higher than a certain amount
%         idx_sel = ones(N,1);
        for ix = 1:NG
%             idx_sel = and(idx_sel,(pkswdthserr(:,ix)<errfact*pkswdths(:,ix)));
            idx_sel = and(idx_sel,(pkswdthserr(:,ix)<wdthmax));
            idx_sel = and(idx_sel,(pkswdths(:,ix)<wdthmax));
            idx_sel = and(idx_sel,mean(pkswdths(:,ix))-pkswdths < selstdfactor*std(pkswdths(:,ix)));
        end
    end
